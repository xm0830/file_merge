...
# 获取表的日期级别的分区
dts=($(hive -e "show partitions ${full_table_name}" 2>/dev/null | awk -F'/' '{if(index($1, "dt") != 0) print $1; else print $2;}' | sort -u | awk -F'=' -v start_dt=$start_dt -v end_dt=$end_dt '{if ($2>=start_dt && $2<end_dt) print $2}'))
for ((i=1; i<=3; i ++))  
do  
  if [[ ${#dts[@]} -eq 0 ]];then
    sleep 5s
